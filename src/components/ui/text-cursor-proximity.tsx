...
import { useMousePositionRef } from "@/src/hooks/use-mouse-position-ref"
  ...
  (
    ...
    const calculateFalloff = (distance: number): number => {
      ...
      switch (falloff) {
        ...
        case "gaussian":
          return Math.exp(-pow(distance / (radius / 2), 2) / 2)
        case "linear":
        default:
    ...
    // Helper function for Math.pow inside calculateFalloff
          ...
          <span key={wordIndex} className="inline-block whitespace-nowrap">
            {word.split("").map((letter) => {
              ...
              const transformedStyles = Object.entries(styles).reduce((acc, [key, value]) => {
                acc[key] = useTransform(proximity, [0, 1], [(value as any).from, (value as any).to])
                return acc
